    ...
    <div className="h-full flex flex-col animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Local Storage Only - Data never leaves device.</p>
        </div>
        <button 
          onClick={() => setShowModal(true)}
          className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/30 transition-all"
        >
      ...
      <div className="glass-panel rounded-2xl overflow-hidden border border-slate-200/60 flex-1 flex flex-col w-full">
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-white/50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                <th className="p-3 md:p-4 font-semibold">Site / App</th>
                <th className="p-3 md:p-4 font-semibold">Username</th>
                <th className="p-3 md:p-4 font-semibold">Password</th>
                <th className="p-3 md:p-4 font-semibold">Strength</th>
                ...
                <tr key={item.id} className="hover:bg-white/40 transition-colors">
                  <td className="p-3 md:p-4">
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded bg-slate-200 flex items-center justify-center mr-3 font-bold text-slate-600 shrink-0">
                        {item.site.charAt(0).toUpperCase()}
                      </div>
                      <span className="font-medium text-slate-800 whitespace-nowrap">{item.site}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-3 md:p-4">
                    <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border whitespace-nowrap
        ...
        {passwords.length === 0 && (
          <div className="p-8 text-center text-slate-400 flex-1 flex flex-col items-center justify-center">
